...
const Login = () => {
  ...
  const [sUser, setsUser] = useState(false)
    ...
    const { user : authuser, userData : authUserData , login, readUser } = useContext(UserContext);
    ...
    const senData = async (user) => {
      ...
      const res = await axios.post('https://api.m3o.com/v1/user/Login', user , {headers});
      ...
      // const {username} = user
      const userRead = await axios.post('https://api.m3o.com/v1/user/Read', {username} , {headers});
    ...
    <form className="form-signin" onSubmit={handleLogin}>
      ...
      <input type="text" className="form-control" name="username" placeholder="username" onChange={handleChange} value={username} required="" autofocus="" />
      <input type="password" className="form-control" name="pass" placeholder="Password" onChange={handleChange} value={password} required=""/>      
      ...
      <p> <Link to="/forget" className='forgpass'>Forget Your Password?</Link></p>
      <Link to="/register" className="forgpass">Register</Link>   
